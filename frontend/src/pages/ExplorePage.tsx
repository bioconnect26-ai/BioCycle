...
import { Search, LayoutGrid, List, SlidersHorizontal, X, Sparkles, BookOpen } from "lucide-react";
...
const difficultyColor: Record<string, { bg: string; text: string; border: string }> = {
  ...
  Intermediate: { bg: "#fffbeb", text: "#92400e", border: "#fcd34d" },
  Advanced:     { bg: "#fff1f2", text: "#991b1b", border: "#fca5a5" },
};
...
function CycleListRow({ cycle, index }: { cycle: any; index: number }) {
  ...
  const diff = difficultyColor[cycle.difficulty] ?? difficultyColor["Beginner"];
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          <p className="text-sm font-semibold truncate" style={{ color: T.text }}>{cycle.title}</p>
          <span className="text-xs px-2 py-0.5 rounded-full font-medium"
            style={{ background: diff.bg, color: diff.text, border: `1px solid ${diff.border}` }}>
            {cycle.difficulty}
          </span>
        </div>
        <p className="text-xs mt-0.5 truncate" style={{ color: T.textSoft }}>{cycle.description}</p>
      </div>
      ...
      <div className="hidden md:flex items-center gap-1.5 flex-shrink-0">
        {cycle.tags.slice(0, 2).map((tag: string) => (
          <span key={tag} className="text-xs px-2.5 py-1 rounded-full"
            style={{ background: T.surfaceAlt, color: T.textSoft, border: `1px solid ${T.border}` }}>
            {tag}
          </span>
        ))}
      </div>
      ...
      {/* Category */}
      <span className="hidden lg:block text-xs flex-shrink-0 font-medium" style={{ color: T.textXSoft }}>
        {cycle.category}
      </span>
      ...
      >
        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke={T.emerald} strokeWidth={2.5}>
...
function AmbientBlobs() {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      <motion.div className="absolute rounded-full blur-[120px] opacity-40"
        style={{ width: 500, height: 500, background: "rgba(5,150,105,0.08)", top: "-8%", right: "-5%" }}
        animate={{ x: [0, 20, 0], y: [0, -16, 0] }} transition={{ duration: 16, repeat: Infinity, ease: "easeInOut" }} />
      <motion.div className="absolute rounded-full blur-[100px] opacity-30"
        style={{ width: 400, height: 400, background: "rgba(8,145,178,0.07)", bottom: "10%", left: "-5%" }}
        animate={{ x: [0, -15, 0], y: [0, 20, 0] }} transition={{ duration: 20, repeat: Infinity, ease: "easeInOut" }} />
    </div>
  );
}
    ...
    <div className="min-h-screen" style={{ background: "var(--background, #f8fafc)" }}>
      ...
      <div className="relative pt-24 pb-16 overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none opacity-[0.035]"
          style={{ backgroundImage: `radial-gradient(${T.emerald} 1.5px, transparent 1.5px)`, backgroundSize: "28px 28px" }} />

        <div className="relative container mx-auto px-4 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            {/* Pill */}
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-5 text-sm font-medium"
              style={{ background: T.emeraldLight, color: T.emerald, border: `1px solid ${T.borderStrong}` }}>
            ...
            <p className="text-lg max-w-xl mx-auto mb-8" style={{ color: T.textSoft }}>
              Search, filter, and discover interactive biology processes — from cell division to the Krebs cycle.
            </p>

            {/* Stats row */}
            <div className="flex items-center justify-center gap-6 md:gap-10 flex-wrap">
              {[
                { label: "Total Cycles", value: cyclesData.length },
                { label: "Categories", value: categories.length - 1 },
                { label: "Interactive", value: "100%" },
              ].map(stat => (
                <div key={stat.label} className="text-center">
                  <div className="font-display text-2xl font-bold" style={{ color: T.emerald }}>{stat.value}</div>
                  <div className="text-xs mt-0.5" style={{ color: T.textXSoft }}>{stat.label}</div>
                </div>
              ))}
            </div>
          </motion.div>
      ...
      {/* ════ SEARCH + FILTERS ════ */}
      <div className="sticky top-[56px] z-40 py-4"
        style={{ background: "rgba(248,250,252,0.92)", backdropFilter: "blur(16px)", borderBottom: `1px solid ${T.border}` }}>
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-3">

            {/* Search bar */}
            <motion.div className="relative flex-1 max-w-lg group"
              ...
              <input
                ...
                onFocus={e => { if (!search) (e.target as HTMLInputElement).style.borderColor = T.emeraldMid; }}
                onBlur={e => { if (!search) (e.target as HTMLInputElement).style.borderColor = T.border; }}
              />
              {search && (
            ...
            <motion.button
              ...
              {(activeCategory !== "All" || activeDifficulty !== "All") && (
                <span className="w-2 h-2 rounded-full" style={{ background: T.emerald }} />
              )}
            </motion.button>
            ...
            <motion.div
              ...
              {([["grid", LayoutGrid], ["list", List]] as const).map(([mode, Icon]) => (
                <button key={mode} onClick={() => setViewMode(mode)}
                  ...
                  style={{
                    background: viewMode === mode ? T.emeraldLight : "transparent",
            ...
            <div className="hidden md:flex items-center gap-2 flex-shrink-0">
              ...
              <span className="text-xs" style={{ color: T.textXSoft }}>results</span>
            </div>
          </div>
            ...
            {showFilters && (
              ...
              >
                <div className="flex flex-wrap gap-6 pt-1 pb-2">
                  {/* Category */}
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: T.textXSoft }}>
                      Category
                    </p>
                    <div className="flex flex-wrap gap-1.5">
                      {categories.map(cat => (
                        <button key={cat} onClick={() => setActiveCategory(cat)}
                          ...
                          style={{
                            background: activeCategory === cat ? T.emerald : T.surface,
                            color: activeCategory === cat ? "white" : T.textMid,
                      ...
                      {difficulties.map(d => {
                        ...
                        return (
                          <button key={d} onClick={() => setActiveDifficulty(d)}
                            className="px-3 py-1.5 rounded-lg text-xs font-medium transition-all hover:scale-[1.03]"
                            style={{
                              background: isActive ? (dc ? dc.bg : T.emeraldLight) : T.surface,
                              color: isActive ? (dc ? dc.text : T.emerald) : T.textMid,
                  ...
                  {hasFilters && (
                    <div className="flex items-end">
                      <button onClick={clearAll}
                        ...
                        style={{ background: "#fff1f2", color: "#ef4444", border: "1.5px solid #fca5a5" }}>
      ...
      <AnimatePresence>
        {hasFilters && (
          <motion.div
            ...
            <span className="text-xs" style={{ color: T.textXSoft }}>Active:</span>
            {search && (
            ...
            {activeDifficulty !== "All" && (
              <span className="flex items-center gap-1 text-xs px-2.5 py-1 rounded-full font-medium"
                style={{ background: T.emeraldLight, color: T.emerald, border: `1px solid ${T.borderStrong}` }}>
                {activeDifficulty}
                <button onClick={() => setActiveDifficulty("All")}><X className="w-3 h-3" /></button>
              </span>
            )}
            <span className="text-xs ml-1" style={{ color: T.textXSoft }}>
              — showing <strong style={{ color: T.emerald }}>{filtered.length}</strong> of {cyclesData.length}
            </span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="container mx-auto px-4 py-8 pb-20">
        <AnimatePresence mode="wait">
          {filtered.length > 0 ? (
            <motion.div key={`${viewMode}-${activeCategory}-${activeDifficulty}`}
              ...
              {viewMode === "grid" ? (
                ...
                <div className="max-w-3xl mx-auto space-y-2.5">
                  {/* List header */}
                  <div className="flex items-center gap-5 px-5 pb-1 text-xs font-semibold uppercase tracking-wider"
            ...
            <motion.div key="empty"
              ...
              <h3 className="font-display text-2xl font-bold mb-2" style={{ color: T.text }}>
                No cycles found
              </h3>
